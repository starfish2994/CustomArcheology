...
public class LocateManager extends BaseManager {
    ...
    protected void onInit() {
        ...
        this.languageFileName = Config.CONFIG_FILES_MINECRAFT_LOCATE_FILE_FILE.asString();
        if (languageFileName == null) {
            return;
        }
        File file = new File(CustomArcheology.plugin.getDataFolder(), languageFileName);
        if (!file.exists()) {
            downloadLocateFile();
        }
    ...
    public void downloadLocateFile() {
        if (!Config.CONFIG_FILES_MINECRAFT_LOCATE_FILE_GENERATE_NEW_ONE.asBoolean()) {
            return;
        }
        Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §fDownloading Minecraft locate file, this will cost some time...");
        String MINECRAFT_VERSION = CustomArcheology.yearVersion +  "." + CustomArcheology.majorVersion + "." + CustomArcheology.minorVersion;
        ...
        try {
            ...
            if (targetVersion == null) {
                Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §cError: Failed to download Minecraft locate file. Reason: Can not get your Minecraft version!");
                return;
            }
            ...
            if (!objects.has("minecraft/lang/" + languageFileName)) {
                Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §cError: Failed to download Minecraft locate file. Reason: Can not find locate file: " + languageFileName + "!");
                return;
            }
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(connection.getInputStream()));
                 FileOutputStream fos = new FileOutputStream(new File(CustomArcheology.plugin.getDataFolder(), languageFileName))) {
            ...
            Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §cError: Failed to download Minecraft locate file. Reason: Internet problem!");
    ...
    public void loadLocateFile() {
        ...
        try {
            FileInputStream fis = new FileInputStream(new File(CustomArcheology.plugin.getDataFolder(), languageFileName));
            this.fileContent = new JSONObject(new JSONTokener(fis));
            fis.close();
        } catch (FileNotFoundException e) {
            Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §cError: Failed to load Minecraft locate file. Reason: Can not find locate file: " + languageFileName + "!");
        } catch (Throwable throwable) {
            Bukkit.getConsoleSender().sendMessage("§x§9§8§F§B§9§8[CustomArcheology] §cError: Failed to load Minecraft locate file. Reason: " + throwable.getMessage() + "!");
            throwable.fillInStackTrace();
        }
    ...
    public static boolean enableThis() {
        return CommonUtil.getMajorVersion(16) && Config.CONFIG_FILES_MINECRAFT_LOCATE_FILE_ENABLED.asBoolean();
    }
}
